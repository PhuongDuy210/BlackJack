import { _decorator, Button, Component, Node, Prefab, instantiate, Label, Vec2, tween, Vec3 } from 'cc';
...
const MAX_STACK_COUNT = 7;
const MAX_STACK_PER_TYPE = 1;
...
export class BetUIManager extends Component {
    ...
    @property(Node)
    private playerArea: Node = null!;

    @property(Node)
    private dealerArea: Node = null!;
    ...
    start() {
        ...
        EventManager.instance.gameEvents.on(GameEvent.PAYOUT, this.payoutPot, this);
        EventManager.instance.gameEvents.on(GameEvent.GAME_RESET, this.resetUI, this);

        this.potOriginalPos = this.potArea.position.clone();
        this.resetUI();
    ...
    protected onDestroy(): void {
        ...
        EventManager.instance.gameEvents.off(GameEvent.PAYOUT, this.payoutPot, this);
    ...
    private payoutPot(payout: number) {
        let targetWorldPos;
        if (payout > 0) {
            targetWorldPos = this.playerArea.getWorldPosition();
        } else {
            targetWorldPos = this.dealerArea.getWorldPosition();
        }
        tween(this.potArea)
            .to(0.3, { worldPosition: targetWorldPos })
            .call(() => {
                this.potArea.active = false;
            })
            .start();
    }
}
